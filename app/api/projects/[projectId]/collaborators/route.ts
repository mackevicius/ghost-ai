...
import {
  enrichCollaborators,
  getOwnerProfile,
} from '@/lib/collaborator-profiles';
...
export async function GET(
  ...
  const ownerAccess = await getProjectAccess(projectId, {
    userId,
    primaryEmail: null,
    verifiedEmails: [],
  });
  const project =
    ownerAccess ??
    (await getProjectAccess(projectId, await getCurrentIdentity()));
  if (!project)
  ...
  const [people, owner] = await Promise.all([
    ...
    getOwnerProfile(record.ownerId),
  ]);
...
export async function POST(
  ...
  try {
    ...
    return Response.json(
      { ...collaborator, displayName: null, imageUrl: null },
      { status: 201 },
    );
  } catch (error) {
    if (
