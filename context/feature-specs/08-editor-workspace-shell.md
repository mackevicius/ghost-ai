...
- Use the supplied three-panel reference: inset rounded project sidebar, central grid-backed canvas placeholder, and AI panel, with narrow gaps between them.
- Both sidebars start open on desktop and toggle independently. On smaller screens, start with the canvas visible and open one sidebar overlay at a time.
- Show the project name and Workspace subtitle in the navbar, and highlight the current project with a tinted row.
- Keep existing theme tokens and placeholder-only behavior; explanatory reference copy is not required.
