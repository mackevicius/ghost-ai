...
7. Add a basic renderer for the custom canvas node type so new nodes are visible.
   ...
   Render the saved shape with a centered label: rounded rectangle, circle, pill, diamond, cylinder, or hexagon. Use inline SVG for diamond, cylinder, and hexagon outlines, retaining the node fill and cyan selection border.
