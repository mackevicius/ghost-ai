...
Show a `People with access` section containing the project owner and collaborators. Label the owner as `Owner` and append `(you)` when viewed by the owner. The owner is not removable. Include the owner even when there are no collaborators or profile enrichment is unavailable.
