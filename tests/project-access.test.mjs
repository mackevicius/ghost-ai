...
function setup(userId = 'owner', user = null, project = null) {
  const queries = [];
  const exports = {};
  const { outputText } = ts.transpileModule(readFileSync(new URL('../lib/project-access.ts', import.meta.url), 'utf8'), {
...
test('identity exposes primary email and only verified collaborator emails', async () => {
  const helper = setup('owner', {
    ...
    emailAddresses: [
      ...
      { emailAddress: 'secondary@example.com', verification: { status: 'verified' } },
  ...
  assert.deepEqual(Array.from(identity.verifiedEmails), ['primary@example.com', 'secondary@example.com']);
});

test('owner access works without an email and returns only sidebar fields', async () => {
  const helper = setup('owner', null, { id: 'room', name: 'Project', ownerId: 'owner' });
  const project = await helper.getProjectAccess('room', await helper.getCurrentIdentity());
  assert.deepEqual(JSON.parse(JSON.stringify(project)), { id: 'room', name: 'Project', isOwner: true });
  assert.deepEqual(helper.queries[0].where, { id: 'room', OR: [{ ownerId: 'owner' }] });
});

test('collaborator query is scoped to the room and verified email, case-insensitively', async () => {
  const helper = setup('member', null, { id: 'room', name: 'Shared', ownerId: 'owner' });
  const project = await helper.getProjectAccess('room', {
    userId: 'member', primaryEmail: 'Member@example.com', verifiedEmails: ['Member@example.com'],
  });
  assert.equal(project.isOwner, false);
  assert.deepEqual(helper.queries[0].where, {
    id: 'room', OR: [
      { ownerId: 'member' },
      { collaborators: { some: { email: { equals: 'Member@example.com', mode: 'insensitive' } } } },
    ],
  });
});
...
test('missing or unauthorized projects return null', async () => {
  const helper = setup();
  assert.equal(await helper.getProjectAccess('unavailable', await helper.getCurrentIdentity()), null);
});