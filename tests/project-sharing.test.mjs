...
function setup({
  ...
  const methods = Object.fromEntries(
    ['findMany', 'findFirst', 'create', 'deleteMany'].map((method) => [
      method,
      async (args) => {
  ...
  const api = load('app/api/projects/[projectId]/collaborators/route.ts', {
    ...
    '@/lib/project-access': {
      ...
      getProjectAccess: async (_projectId, identity) =>
        access && (userId === ownerId || identity.verifiedEmails.length)
          ? { isOwner: userId === ownerId }
          : null,
    },
    '@/lib/collaborator-profiles': {
      enrichCollaborators: async (people) =>
        people.map((person) => ({
          ...person,
          displayName: 'Member',
...
const request = (method, body) =>
  new Request('http://localhost/api/projects/room/collaborators', {
    method,
...
test('owners and collaborators can list, with server-derived permissions', async () => {
  for (const userId of ['owner', 'member']) {
    ...
    assert.deepEqual(
      api.calls.find((call) => call.method === 'findMany').args.where,
      { projectId: 'room' },
    );
    assert.equal(
      api.calls.some((call) => call.method === 'identity'),
      userId !== 'owner',
    );
  }
});
...
test('collaborators cannot invite or remove even with forged owner input', async () => {
  ...
  assert.equal(
    (
      await api.POST(
        request('POST', { email: 'new@example.com', isOwner: true }),
        context,
      )
    ).status,
    403,
  ...
  assert.equal(
    (
      await api.DELETE(
        request('DELETE', { collaboratorId: 'member-id', isOwner: true }),
        context,
      )
    ).status,
    403,
...
test('invite validates and normalizes email without accepting project overrides', async () => {
  ...
  const response = await api.POST(
    request('POST', { email: ' Member@Example.com ', projectId: 'other' }),
    context,
  );
  assert.equal(response.status, 201);
  assert.equal((await response.json()).email, 'member@example.com');
  assert.deepEqual(api.calls[1].args.data, {
    projectId: 'room',
...
test('duplicate invites and racing duplicate writes return 409', async () => {
  for (const options of [{ existing: true }, { collision: true }]) {
    ...
    assert.equal(
      (
        await api.POST(
          request('POST', { email: 'member@example.com' }),
          context,
        )
      ).status,
      409,
...
test('remove is scoped to collaborator, room, and owner', async () => {
  ...
  assert.equal(
    (
      await api.DELETE(
        request('DELETE', { collaboratorId: 'member-id' }),
        context,
      )
    ).status,
    204,
  );
  assert.deepEqual(api.calls[0].args.where, {
    id: 'member-id',
    projectId: 'room',
  ...
  assert.equal(
    (
      await otherRoom.DELETE(
        request('DELETE', { collaboratorId: 'elsewhere' }),
        context,
      )
    ).status,
    404,
...
test('missing projects reject mutations', async () => {
  ...
  assert.equal(
    (await api.POST(request('POST', { email: 'member@example.com' }), context))
      .status,
    404,
  );
  assert.equal(
...
test('Clerk enriches matching verified emails with name and image, falling back for unknown emails', async () => {
  ...
  const people = await enrichCollaborators([
...
test('slow Clerk enrichment falls back within its deadline', async () => {
  let deadline;
  const { enrichCollaborators } = load(
    'lib/collaborator-profiles.ts',
    {
      'server-only': {},
  ...
  const pending = enrichCollaborators([
    { id: 'member', email: 'member@example.com' },
  ]);
  deadline();
...
test('empty collaborator lists do not contact Clerk', async () => {
  const { enrichCollaborators } = load('lib/collaborator-profiles.ts', {
    'server-only': {},
    '@clerk/nextjs/server': {
      clerkClient: () => assert.fail('Clerk should not be called'),
    },
  });
...
test('copy writes the room URL and clears Copied feedback after two seconds', async () => {
  ...
  let timeout;
  let copiedText;
  const { useProjectSharing: runSharing } = load(
    'hooks/use-project-sharing.ts',
    {
      react: {
        useState: (initial) => {
          const index = slot++;
