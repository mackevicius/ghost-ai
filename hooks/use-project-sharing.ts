...
export function useProjectSharing(projectId: string) {
    ...
    async function load() {
      try {
        ...
        if (
          ...
          !body.collaborators.every(isCollaborator) ||
          !('isOwner' in body) ||
          typeof body.isOwner !== 'boolean' ||
          !('owner' in body) ||
          !isCollaborator(body.owner)
        )
  ...
  async function mutate(
    method: 'POST' | 'DELETE',
    body: { email: string } | { collaboratorId: string },
  ) {
  ...
  async function copyLink() {
    ...
    try {
      await navigator.clipboard.writeText(
        `${window.location.origin}/editor/${encodeURIComponent(projectId)}`,
      );
      setCopied(true);
      if (copyTimer.current) clearTimeout(copyTimer.current);
  ...
  return {
    ...
    remove: (collaboratorId: string) => mutate('DELETE', { collaboratorId }),
    copyLink,
  };
}
