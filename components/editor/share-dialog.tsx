...
export function ShareDialog({
  ...
  return (
    <Dialog
      open
      onOpenChange={(open) => {
        if (!open && !sharing.pending) onClose();
      }}
    >
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-3xl bg-surface text-copy-primary">
          ...
          >
            ...
            <div className="flex min-w-0 gap-2">
              <Input
                ...
                disabled={sharing.pending || sharing.loading}
                className="min-w-0 flex-1 rounded-xl"
              />
              <Button
                type="submit"
                disabled={
                  sharing.pending || sharing.loading || !sharing.email.trim()
                }
              >
        ...
        {sharing.error && (
          <div className="grid gap-2">
            <p role="alert" className="text-sm wrap-anywhere text-error">
              {sharing.error}
            </p>
            <Button
              variant="outline"
        ...
        <div aria-busy={sharing.loading || sharing.pending} className="min-w-0">
          <h3 className="mb-2 text-sm font-medium text-copy-primary">
            People with access
          </h3>
          {sharing.loading ? (
            <p
              role="status"
              className="flex items-center gap-2 py-4 text-sm text-copy-muted"
            >
            ...
            <ul className="max-h-72 space-y-3 overflow-y-auto py-2">
              {sharing.owner && (
                <li className="flex min-w-0 items-center gap-3">
                  {sharing.owner.imageUrl ? (
                    <Image
                      src={sharing.owner.imageUrl}
                      alt=""
                      width={32}
                      height={32}
                      unoptimized
                    ...
                    <UserRound
                      className="h-8 w-8 shrink-0 text-copy-muted"
                      aria-hidden="true"
                    />
                  )}
                  <div className="min-w-0 flex-1 text-sm wrap-anywhere">
                    ...
                    {sharing.owner.email && (
                      <p className="text-copy-muted">{sharing.owner.email}</p>
                    )}
                  </div>
                ...
                <li key={person.id} className="flex min-w-0 items-center gap-3">
                  ...
                  {sharing.isOwner && (
                    <Button
                      ...
                      onClick={() => void sharing.remove(person.id)}
        ...
        {sharing.isOwner && (
          ...
          >
            <span aria-live="polite" className="inline-flex items-center gap-2">
              ...
              {sharing.copied ? 'Copied!' : 'Copy project link'}
            </span>
          </Button>
        )}
      </DialogContent>
